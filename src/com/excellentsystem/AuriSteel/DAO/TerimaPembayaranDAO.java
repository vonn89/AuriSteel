...
public class TerimaPembayaranDAO {
    ...
    public static TerimaPembayaran getByTipeKeuanganAndCatatanAndStatus(Connection con, String tipeKeuangan, String noPemesanan, String status) throws Exception {
        String sql = "select * from tt_terima_pembayaran where tipe_keuangan = '" + tipeKeuangan + "' and catatan = '" + noPemesanan + "' and status = '" + status + "' ";
        PreparedStatement ps = con.prepareStatement(sql);
        ResultSet rs = ps.executeQuery();
        TerimaPembayaran t = null;
        while (rs.next()) {
            t = new TerimaPembayaran();
            t.setNoTerimaPembayaran(rs.getString(1));
            t.setTglTerima(rs.getDate(2).toString() + " " + rs.getTime(2).toString());
            t.setNoPiutang(rs.getString(3));
            t.setJumlahPembayaran(rs.getDouble(4));
